...
import shared.pages.elements.DrawingPageElementIds._
import shared.resources.css.DrawingPageStyles
...
object DrawingPage extends Page {
  ...
  private val pageBody = body(cls := DrawingPageStyles.body.name)(
    ...
    canvas(id := canvasId.value, cls := DrawingPageStyles.canvas.name, widthA := 600, heightA := 400)(canvasNotSupportedText)
  )
